...
function App() {
  ...
  function task3 (event) {
    ...
    fetch("http://localhost/api.php", {
      ...
      .then(response => response.text())
      .then(response => response > 0 ? setT3(filename) : setT3(false))
  ...
  return (
    <div>
      ...
      <div>
        ...
        <p>{t3 !== false ? <a href={`http://localhost/files/${t3}`} download>Скачать</a> : "" }</p>
      </div>
      ...
      <div>
        ...
        <form action="" onSubmit = {task5}>
          <button type="sumbit">Push</button>
		    ...
		    <ul>{ 
            t5.map((item, index) => (
              <li key={index}>{item.ccy} - {item.buy}</li>
            ))
         }</ul>
      </div>
